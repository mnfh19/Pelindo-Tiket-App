...
class BookingController extends Controller
{
    ...
    public function book(Request $r){
        ...
        if($book){
            ...
            $getKend = TiketKendaraanModel::where('id_tiket_kendaraan', $r->id_tiket_kendaraan)->first();

            return response()->json([
                'respon' => true,
                'booking' => $book->id_booking,
                'kendaraan' => $getKend->harga,
            ]);
        }
    ...
    public function setPembayaran(Request $r){
        ...
        BookingModel::where('id_booking', '=', $r->id_booking)->update(['status_booking' => 1, 'status_bayar' => 1, 'bukti_pembayaran' => $r->bukti]);
        return response()->json([
            'respon' => true,
    ...
    public function all($id){
        ...
        foreach ($get as $d) {
            ...
            if($d->id_tiket != 0 &&$d->id_tiket_kendaraan != 0){
                $pick = BookingModel::select('booking.*', 'jadwal.*', 'tiket.*', 'kapal.*', 'rute.*', 'tiket_kendaraan.*')
                                        ...
                                        ->where('id_booking', $d->id_booking)->get();
            }
            ...
            if($d->id_tiket_kendaraan != 0 && $d->id_tiket == 0 ){
    ...
    public function batalkanBooking($id){
        BookingModel::where('id_booking', '=', $id)->update(['status_booking' => 5]);
        return true;
    }
